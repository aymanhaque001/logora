...
The design language signals effort: epistemic vocabulary, structured forms, AI classification. People don't want to _debate_, they want to _talk_. Even users who would enjoy the depth may be put off by how serious and academic it looks before they experience the payoff.
...
When AI correctly classifies a node, labels a nuance, or generates a concept summary, the user does not notice. AI is only visible when it is _wrong_, which creates a negative impression disproportionate to its actual value. The system needs better surface-level feedback for when AI is helping.
