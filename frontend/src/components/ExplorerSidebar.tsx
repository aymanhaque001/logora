...
function ExplorerNode({
  ...
  const summary = n.ai_summary ?? (n.content.length > 60 ? n.content.slice(0, 60) + "\u2026" : n.content);
  const dotColor = TYPE_DOTS[n.node_type] ?? "#6b7080";

  return (
    <div>
      <div
        className={`group flex items-start gap-1.5 px-2 py-1.5 cursor-pointer rounded-md transition-colors ${
          isSelected ? "bg-accent-muted text-text-primary" : "hover:bg-surface-3 text-text-secondary"
        }`}
      ...
      >
        ...
        <button
          onClick={(e) => { e.stopPropagation(); if (hasKids) toggleCollapse(n.id); }}
        ...
        {/* Type dot */}
        <span className="shrink-0 w-2 h-2 rounded-full mt-1.5" style={{ background: dotColor }} />

        {/* Summary text */}
        <div className="min-w-0 flex-1">
          <p className="text-xs leading-snug line-clamp-2">{summary}</p>
          <div className="flex items-center gap-2 mt-0.5">
            <span className="text-[10px] text-text-tertiary">{n.author_display_name}</span>
            {n.track_name && (
              <span className="text-[10px] text-text-tertiary opacity-60">· {n.track_name}</span>
            )}
            {item.children.length > 0 && (
              <span className="text-[10px] text-text-tertiary opacity-50">{item.children.length} replies</span>
            )}
          </div>
        </div>
      </div>
...
export function ExplorerSidebar({ graphNodes, graphEdges, tracks, onNodeClick, selectedId }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterTrack !== "all") nodes = nodes.filter((n) => n.track_id === filterTrack);
    if (searchTerm.trim()) {
  ...
  const tree = useMemo(() => buildTree(filtered, graphEdges), [filtered, graphEdges]);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-2.5 border-b border-border flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-text-secondary uppercase tracking-wider">Argument Map</span>
        </div>
        <span className="text-[10px] text-text-tertiary">{graphNodes.length}</span>
      </div>
      ...
      <div className="px-3 py-2 border-t border-border-subtle">
        <div className="flex flex-wrap gap-x-3 gap-y-1">
          {(Object.entries(TYPE_DOTS) as [NodeType, string][]).map(([type, color]) => (
            <div key={type} className="flex items-center gap-1 text-[10px] text-text-tertiary">
