...
export function CreateTopic() {
  ...
  return (
    <div className='max-w-xl mx-auto px-6 py-10 animate-fade-in'>
      <button
        onClick={() => navigate('/')}
        className='flex items-center gap-1.5 text-sm text-text-tertiary hover:text-text-secondary transition mb-6'
      >
        <ArrowLeft size={14} /> Back
      </button>

      <h1 className='text-xl font-light text-text-primary mb-1 lowercase tracking-wide'>
        new debate
      </h1>
      <p className='text-sm font-light text-text-tertiary mb-6'>
        frame a clear, neutral question for structured discourse.
      </p>
      ...
      >
        ...
        {error && (
          <p className='text-sm text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2 animate-slide-down'>
            {error}
          </p>
        )}
