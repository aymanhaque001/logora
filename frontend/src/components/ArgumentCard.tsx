...
import {
  ExternalLink, AlertCircle, ChevronDown, ChevronRight,
  Trash2, Edit3, Flag, ShieldCheck, HandshakeIcon, GitBranch, Merge,
  RefreshCw, Moon, Zap, MessageSquare, CornerDownRight,
} from "lucide-react";
...
const NODE_ACCENT: Record<string, string> = {
  ...
  synthesis:     "#22c55e",
  reframe:       "#a855f7",
  open_question: "#6b7080",
  concession:    "#14b8a6",
};

const STATE_CONFIG: Record<string, { dot: string; label: string; terminal?: boolean; icon?: React.ReactNode }> = {
  unchallenged: { dot: "bg-text-tertiary", label: "Unchallenged" },
  engaged:   { dot: "bg-blue-400",   label: "Engaged",   icon: <Zap size={10} /> },
  refined:   { dot: "bg-indigo-400", label: "Refined",   icon: <RefreshCw size={10} /> },
  branched:  { dot: "bg-purple-400", label: "Branched",  icon: <GitBranch size={10} /> },
  merged:    { dot: "bg-amber-400",  label: "Merged",    terminal: true, icon: <Merge size={10} /> },
  conceded:  { dot: "bg-teal-400",   label: "Conceded",  terminal: true, icon: <HandshakeIcon size={10} /> },
...
const NUANCE_LABELS: Record<string, string> = {
  temporal: "temporal", geographic: "geographic", scale: "scale",
  conditional: "conditional", population_specific: "population", contested_empirically: "contested",
};

interface TransitionOption {
  state: string; label: string; description: string; author_only: boolean;
}
...
export function ArgumentCard({
  node, topicId, onReply, onDelete, onEdit, onTransition,
  onTransitionSuccess, currentUserId, depth = 0, highlighted = false,
}: Props) {
  const [showStateMenu, setShowStateMenu] = useState(false);
  const [transitions, setTransitions] = useState<TransitionOption[]>([]);
  const [loadingTransitions, setLoadingTransitions] = useState(false);
  const [confirmState, setConfirmState] = useState<TransitionOption | null>(null);
  ...
  useEffect(() => {
    ...
    if (showStateMenu) document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, [showStateMenu]);

  const openTransitionMenu = async () => {
    if (showStateMenu) { setShowStateMenu(false); setConfirmState(null); return; }
    ...
    >
      ...
      <div className="flex gap-3">
        ...
        <div className="flex flex-col items-center shrink-0">
          <div className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-semibold text-white" style={{ background: accent }}>
        ...
        <div className="flex-1 min-w-0 pb-4">
          ...
          <div className="flex items-center gap-2 mb-1">
            <span className="text-sm font-medium text-text-primary">{node.author.display_name}</span>
            {node.author.is_verified_expert && (
              <span className="px-1.5 py-0.5 bg-violet-500/15 text-violet-400 rounded text-[10px] font-medium">{node.author.expert_domain}</span>
            )}
            <span className="text-[10px] text-text-tertiary flex items-center gap-0.5">
              <ShieldCheck size={9} /> {Math.round(node.author.credibility_score)}
            </span>
          ...
          <div className="flex items-center gap-1.5 mb-2">
            <span className="text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded" style={{ color: accent, background: `${accent}20` }}>
              {node.node_type.replace("_", " ")}
            </span>
            {node.nuance_tags.map((tag) => (
              <span key={tag} className="px-1.5 py-0.5 rounded text-[10px] bg-surface-3 text-text-tertiary font-medium">
          ...
          {/* Terminal banner */}
          {isTerminal && (
            <div className={`mb-2 px-3 py-1.5 text-xs font-medium rounded flex items-center gap-1.5 ${
              node.state === "conceded"
          ...
          <div className="flex items-center gap-1 mt-2.5 -ml-1.5">
                ...
                {showStateMenu && (
                  <div className="absolute right-0 bottom-full mb-2 bg-surface-2 border border-border rounded-lg shadow-xl z-20 min-w-[240px] overflow-hidden animate-scale-in">
                    <div className="px-3 py-2.5 border-b border-border-subtle">
                      <div className="text-xs font-medium text-text-primary">Transition State</div>
                      <div className="text-[10px] text-text-tertiary mt-0.5">
                        Current: <span className="font-medium text-text-secondary">{stateConfig.label}</span>
                      </div>
                    </div>
                    ...
                    {loadingTransitions ? (
                      ...
                      <div className="p-3 animate-fade-in">
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          <span className="text-sm font-semibold text-text-primary">{confirmState.label}</span>
                        </div>
                        <p className="text-xs text-text-tertiary leading-relaxed mb-2.5">{confirmState.description}</p>
                        {confirmState.author_only && (
                          <p className="text-[10px] text-amber-400 bg-amber-500/10 rounded px-2 py-1 mb-2.5 font-medium">Author only</p>
                        )}
                        <div className="flex gap-2">
                          <button onClick={() => setConfirmState(null)} className="flex-1 px-3 py-1.5 text-xs btn-secondary rounded-md">Back</button>
                          <button onClick={handleTransitionConfirm} className="flex-1 px-3 py-1.5 text-xs btn-primary rounded-md">Confirm</button>
                        </div>
                      </div>
                    ) : transitions.length === 0 ? (
                      <div className="px-3 py-4 text-xs text-text-tertiary text-center">No transitions available</div>
                    ) : (
                      <div className="py-1">
                        {transitions.map((t) => (
                          <button
                          ...
                          >
                            <span className="mt-0.5 shrink-0">{TRANSITION_ICONS[t.state] ?? <span className={`inline-block w-2.5 h-2.5 rounded-full ${STATE_CONFIG[t.state]?.dot ?? "bg-text-tertiary"}`} />}</span>
                            <div className="min-w-0">
                              <div className="text-xs font-medium text-text-secondary group-hover:text-text-primary flex items-center gap-1.5">
                                {t.label}
                                {t.author_only && <span className="text-[9px] px-1 py-0.5 bg-surface-4 text-text-tertiary rounded">author</span>}
                              </div>
                              <div className="text-[10px] text-text-tertiary leading-snug mt-0.5 line-clamp-2">{t.description}</div>
                            </div>
                          </button>
                        ))}
                      </div>
            ...
            {isOwner && onEdit && !isTerminal && (
              <button onClick={() => onEdit(node)} className="text-xs p-1.5 rounded-md text-text-tertiary hover:text-accent hover:bg-accent-muted transition-colors" title="Edit">
                <Edit3 size={11} />
              </button>
            )}
            {isOwner && onDelete && node.children_count === 0 && !isTerminal && (
              <button
                onClick={() => { if (window.confirm("Delete this argument?")) onDelete(node.id); }}
                className="text-xs p-1.5 rounded-md text-text-tertiary hover:text-red-400 hover:bg-red-500/10 transition-colors" title="Delete"
              >
                <Trash2 size={11} />
              </button>
            )}
