...
def rag_briefing(
    ...
    Uses graph-aware retrieval to provide better context than naive summarization.
    """
    ctx = retrieve_context(
        query=query,
        topic_id=topic_id,
    ...
    context_text = "\n\n".join([
        f"[{n.get('node_type', '?').upper()}] (source: {n.get('source', '?')}, sim: {n.get('similarity', 'N/A')}):\n{n['content'][:400]}"
        for n in ctx["merged_context"][:20]
    ])
