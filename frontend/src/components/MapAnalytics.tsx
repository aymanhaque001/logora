...
import { GraphNode, GraphEdge, NodeType, ArgumentState, EdgeRelationship } from "../types";
...
function HBar({ label, value, total, color }: { label: string; value: number; total: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <span className="text-text-secondary flex-1 truncate capitalize">{label.replace(/_/g, " ")}</span>
      <span className="text-text-tertiary tabular-nums w-6 text-right">{value}</span>
...
export function MapAnalytics({ nodes, edges, onHighlightNode }: Props) {
  const stats = useMemo(() => {
    ...
    for (const e of edges) byEdge[e.relationship_type] = (byEdge[e.relationship_type] ?? 0) + 1;

    // Track distribution
    const byTrack: Record<string, { name: string; count: number }> = {};
    for (const n of nodes) {
      const tid = n.track_id ?? "__none__";
      if (!byTrack[tid]) byTrack[tid] = { name: n.track_name ?? "Untracked", count: 0 };
      byTrack[tid].count++;
    }
    ...
    function getDepth(id: string): number {
      let d = 0;
      let cur = id;
      while (parentOf.has(cur)) { cur = parentOf.get(cur)!; d++; if (d > 50) break; }
      return d;
    }
    ...
    const contentious = (byEdge["challenges"] ?? 0) + (byEdge["contradicts"] ?? 0);
    const supportive = (byEdge["supports"] ?? 0) + (byEdge["synthesizes"] ?? 0);

    return {
      byType, byState, byEdge, byTrack,
      sourcedCount, sourcedRatio: percent(sourcedCount, nodes.length),
      mostConnected, maxDepth, avgDepth,
      contentious, supportive,
    };
  }, [nodes, edges]);
  ...
  return (
    <div className="flex flex-col gap-4 p-3 text-xs overflow-y-auto h-full">
      {/* Key Metrics */}
      <div>
        <h4 className="text-[10px] font-semibold text-text-tertiary uppercase tracking-wider mb-2">Overview</h4>
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-surface-2 rounded-md px-3 py-2">
            <div className="text-lg font-bold text-text-primary tabular-nums">{totalNodes}</div>
            <div className="text-text-tertiary">Arguments</div>
          </div>
          <div className="bg-surface-2 rounded-md px-3 py-2">
            <div className="text-lg font-bold text-text-primary tabular-nums">{totalEdges}</div>
            <div className="text-text-tertiary">Connections</div>
          </div>
          <div className="bg-surface-2 rounded-md px-3 py-2">
            <div className="text-lg font-bold text-text-primary tabular-nums">{stats.sourcedRatio}%</div>
            <div className="text-text-tertiary">Sourced</div>
          </div>
          <div className="bg-surface-2 rounded-md px-3 py-2">
            <div className="text-lg font-bold text-text-primary tabular-nums">{stats.maxDepth}</div>
      ...
      {/* Discourse Health */}
      <div>
        <h4 className="text-[10px] font-semibold text-text-tertiary uppercase tracking-wider mb-2">Discourse Health</h4>
        <div className="bg-surface-2 rounded-md p-3 space-y-2">
          <div className="flex justify-between">
            <span className="text-text-secondary">Avg depth</span>
            <span className="text-text-primary font-medium tabular-nums">{stats.avgDepth}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-text-secondary">Contentious edges</span>
            <span className="text-red-400 font-medium tabular-nums">{stats.contentious}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-text-secondary">Supportive edges</span>
            <span className="text-emerald-400 font-medium tabular-nums">{stats.supportive}</span>
          </div>
          {/* Contention gauge */}
      ...
      <div>
        <h4 className="text-[10px] font-semibold text-text-tertiary uppercase tracking-wider mb-2">By Type</h4>
      ...
      {/* State Distribution */}
      <div>
        <h4 className="text-[10px] font-semibold text-text-tertiary uppercase tracking-wider mb-2">By State</h4>
        <div className="space-y-1.5">
      ...
      {/* Edge Distribution */}
      <div>
        <h4 className="text-[10px] font-semibold text-text-tertiary uppercase tracking-wider mb-2">Relationships</h4>
        <div className="space-y-1.5">
      ...
      {Object.keys(stats.byTrack).length > 0 && (
        <div>
          <h4 className="text-[10px] font-semibold text-text-tertiary uppercase tracking-wider mb-2">By Track</h4>
          <div className="space-y-1.5">
      ...
      {stats.mostConnected.length > 0 && (
        <div>
          <h4 className="text-[10px] font-semibold text-text-tertiary uppercase tracking-wider mb-2">Most Connected</h4>
              ...
              >
                <div className="flex items-center gap-2">
                  <span
                    ...
                    style={{ background: NODE_COLORS[node!.node_type]?.accent ?? "#6b7080" }}
                  />
                  ...
                  <span className="text-accent font-bold tabular-nums shrink-0">{degree}</span>
                </div>
                <div className="text-[10px] text-text-tertiary mt-0.5 ml-4">
                  {node!.author_display_name} · {node!.node_type.replace("_", " ")}
                </div>
              </button>
            ))}
          </div>
      ...
      <div>
        <h4 className="text-[10px] font-semibold text-text-tertiary uppercase tracking-wider mb-2">Type Composition</h4>
        <div className="flex justify-center">
          <DonutChart data={stats.byType} colors={Object.fromEntries(Object.entries(NODE_COLORS).map(([k, v]) => [k, v.accent]))} total={totalNodes} />
        </div>
      </div>
    </div>
  );
...
function DonutChart({ data, colors, total }: { data: Record<string, number>; colors: Record<string, string>; total: number }) {
  const R = 60;
  const STROKE = 14;
  const C = 2 * Math.PI * R;
  let offset = 0;
  ...
  return (
    <svg width={160} height={160} viewBox="0 0 160 160">
      <circle cx={80} cy={80} r={R} fill="none" stroke="#22262e" strokeWidth={STROKE} />
      ...
      <text x={80} y={76} textAnchor="middle" fill="#e2e4e9" fontSize={20} fontWeight={700} fontFamily="Inter, system-ui, sans-serif">
        {total}
      </text>
      <text x={80} y={94} textAnchor="middle" fill="#6b7080" fontSize={10} fontFamily="Inter, system-ui, sans-serif">
        arguments
      </text>
    </svg>
  );
