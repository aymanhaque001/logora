...
function TreeNode({
  node, childMap, topicId, onReply, onDelete, onTransition,
  onTransitionSuccess, currentUserId, depth, highlightedId,
  replyToId, onCancelReply, onReplySuccess,
}: TreeNodeProps) {
  ...
  return (
    <div>
      <ArgumentCard
        node={node} topicId={topicId} onReply={onReply} onDelete={onDelete}
        onTransition={onTransition} onTransitionSuccess={onTransitionSuccess}
        currentUserId={currentUserId} depth={depth} highlighted={highlightedId === node.id}
      />
        ...
        <div className="mt-1">
          ...
          {childrenVisible && (
            <div className="ml-11 pl-4 border-l border-border-subtle space-y-1">
              {children.map((child) => (
                <TreeNode
                  key={child.id} node={child} childMap={childMap} topicId={topicId}
                  onReply={onReply} onDelete={onDelete} onTransition={onTransition}
                  onTransitionSuccess={onTransitionSuccess} currentUserId={currentUserId}
                  depth={depth + 1} highlightedId={highlightedId}
                  replyToId={replyToId} onCancelReply={onCancelReply} onReplySuccess={onReplySuccess}
                />
              ))}
            </div>
          )}
...
export function TopicDetail() {
  ...
  const { data: topic } = useQuery<Topic>({ queryKey: ["topic", topicId], queryFn: () => getTopic(topicId) });
  const { data: tracks = [] } = useQuery<DiscourseTrack[]>({ queryKey: ["tracks", topicId], queryFn: () => getTracks(topicId) });
  ...
  const deleteArgMutation = useMutation({ mutationFn: (argumentId: string) => deleteArgument(topicId, argumentId), onSuccess: () => handleSuccess() });
  const transitionMutation = useMutation({
    mutationFn: ({ argumentId, newState }: { argumentId: string; newState: string }) =>
      transitionArgumentState(topicId, argumentId, { new_state: newState }),
    onSuccess: () => handleSuccess(),
  });
  const deleteTopicMutation = useMutation({ mutationFn: () => deleteTopic(topicId), onSuccess: () => navigate("/") });
  const archiveTopicMutation = useMutation({ mutationFn: () => archiveTopic(topicId), onSuccess: () => handleSuccess() });
  ...
  return (
    <div className="animate-fade-in flex flex-col" style={{ height: "calc(100vh - 3rem)" }}>
      {/* Topic header */}
      <div className="px-5 py-3 border-b border-border bg-surface-1 shrink-0">
        <div className="max-w-[1400px] mx-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              <button onClick={() => navigate("/")} className="text-text-tertiary hover:text-text-secondary transition shrink-0">
                <ArrowLeft size={16} />
              </button>
              <div className="min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  {topic.tags.map((tag) => (
                    <span key={tag} className="flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 bg-surface-3 text-text-tertiary rounded font-medium">
                      <Tag size={8} /> {tag}
                    </span>
                  ))}
                  {topic.location && (
                  ...
                  <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${
                    topic.status === "active" ? "bg-emerald-500/15 text-emerald-400" : "bg-surface-3 text-text-tertiary"
                  }`}>{topic.status}</span>
                </div>
                <h1 className="text-sm font-semibold text-text-primary truncate">{topic.canonical_question}</h1>
              </div>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              ...
              {user && topic.created_by === user.id && topic.node_count === 0 && (
                <button
                  onClick={() => { if (window.confirm("Delete permanently?")) deleteTopicMutation.mutate(); }}
              ...
              >
                {explorerOpen ? <PanelLeftClose size={15} /> : <PanelLeft size={15} />}
              </button>
            </div>
          </div>
          {topic.description && <p className="mt-1 text-xs text-text-tertiary max-w-2xl ml-7">{topic.description}</p>}
        </div>
      </div>
          ...
          <div className="max-w-2xl mx-auto px-5 py-5">
            ...
            {showForm ? (
              <div className="mb-5 animate-slide-down">
                <SubmitArgumentForm topicId={topicId} onCancel={() => setShowForm(false)} onSuccess={handleSuccess} />
              </div>
            ) : (
              <button
                onClick={() => { setShowForm(true); setReplyToId(null); }}
            ...
            {rootNodes.length === 0 && !showForm && (
              <div className="text-center py-20">
                <GitBranch size={24} className="text-text-tertiary mx-auto mb-2 opacity-30" />
                <p className="text-sm text-text-tertiary">No arguments yet. Start the debate.</p>
              </div>
            )}

            <div className="space-y-1">
              {rootNodes.map((root, i) => (
                <div key={root.id} className="animate-slide-up" style={{ animationDelay: `${i * 0.03}s`, animationFillMode: "backwards" }}>
                  <TreeNode
                    node={root} childMap={childMap} topicId={topicId}
                    onReply={handleReply} onDelete={(id) => deleteArgMutation.mutate(id)}
                    onTransition={(id, state) => transitionMutation.mutate({ argumentId: id, newState: state })}
                    onTransitionSuccess={showToast} currentUserId={user?.id}
                    depth={0} highlightedId={highlightedId}
                    replyToId={replyToId} onCancelReply={() => setReplyToId(null)} onReplySuccess={handleSuccess}
                  />
                </div>
              ))}
            </div>
        ...
        <aside className="w-80 border-l border-border bg-surface-1 shrink-0 overflow-y-auto hidden xl:block">
          <div className="p-4">
            ...
            >
              ...
              {briefingOpen ? <ChevronDown size={13} className="text-text-tertiary" /> : <ChevronRight size={13} className="text-text-tertiary" />}
            </button>
            {briefingOpen && (
              <div className="animate-slide-down">
                {briefing ? <BriefingRoom briefing={briefing} /> : (
                  <div className="space-y-2">
                    {[1, 2].map((i) => <div key={i} className="h-16 bg-surface-2 rounded animate-pulse" />)}
                  </div>
                )}
              </div>
            )}
          ...
          <div className="px-4 pb-4">
            <h3 className="text-xs font-semibold text-text-secondary uppercase tracking-wider mb-2">Tracks ({tracks.length})</h3>
            <div className="space-y-1">
              {tracks.map((t) => (
                <div key={t.id} className="flex items-center justify-between text-xs text-text-secondary bg-surface-2 rounded-md px-3 py-2">
                  <span className="font-medium">{t.name}</span>
                  <span className="text-text-tertiary">{t.node_count}</span>
                </div>
              ))}
              {tracks.length === 0 && <p className="text-xs text-text-tertiary">Tracks appear as arguments are submitted.</p>}
            </div>
          </div>
        </aside>
      </div>
