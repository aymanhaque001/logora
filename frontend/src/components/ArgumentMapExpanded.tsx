...
import { GraphNode, GraphEdge, NodeType, EdgeRelationship, DiscourseTrack, ArgumentState } from "../types";
import { NODE_COLORS } from "./ArgumentGraph";
import { MapAnalytics } from "./MapAnalytics";
import {
  X, Maximize2, BarChart3, Layers, GitBranch, Clock,
  ChevronDown, ChevronRight, Search, Focus, Palette,
  Network, TreePine, Columns3, Radio,
} from "lucide-react";
...
const LAYOUT_OPTIONS: { value: LayoutMode; label: string; icon: typeof TreePine }[] = [
...
const STATE_COLORS: Record<ArgumentState, { bg: string; border: string; text: string; accent: string }> = {
  unchallenged: { bg: "#451a03", border: "#f59e0b", text: "#fde68a", accent: "#f59e0b" },
  engaged:      { bg: "#1e1b4b", border: "#6366f1", text: "#c7d2fe", accent: "#6366f1" },
  refined:      { bg: "#2e1065", border: "#818cf8", text: "#ddd6fe", accent: "#818cf8" },
  branched:     { bg: "#3b0764", border: "#a855f7", text: "#e9d5ff", accent: "#a855f7" },
  merged:       { bg: "#052e16", border: "#22c55e", text: "#bbf7d0", accent: "#22c55e" },
  conceded:     { bg: "#042f2e", border: "#14b8a6", text: "#99f6e4", accent: "#14b8a6" },
  dormant:      { bg: "#1a1d23", border: "#6b7080", text: "#9ca0ab", accent: "#6b7080" },
};
...
function applyClusterLayout(nodes: Node[], edges: Edge[], graphNodes: GraphNode[]): Node[] {
  if (nodes.length === 0) return nodes;
  ...
  for (const [, group] of trackGroups) {
    ...
    for (const n of groupNodes) {
      const pos = g.node(n.id);
      allPositioned.set(n.id, { x: pos.x - NODE_W / 2 + xOffset, y: pos.y - NODE_H / 2 });
...
function getNodeColors(
  ...
  switch (colorMode) {
    ...
    case "connectivity": {
      ...
      const maxDeg = Math.max(1, ...allNodes.map((n) => edges.filter((e) => e.source === n.id || e.target === n.id).length));
      const t = degree / maxDeg;
      // Low connectivity = dim, high = bright
      if (t < 0.3) return { bg: "#1a1d23", border: "#3a3f4a", text: "#9ca0ab", accent: "#6b7080" };
      if (t < 0.6) return { bg: "#1e1b4b", border: "#6366f1", text: "#c7d2fe", accent: "#6366f1" };
      return { bg: "#312e81", border: "#818cf8", text: "#e0e7ff", accent: "#818cf8" };
    }
  }
}
...
function ExpandedNodeCard({ data }: NodeProps<ExpandedNodeData>) {
  const { graphNode: n, colors: c, childCount, collapsed, onToggle, onNodeClick, selected, degree } = data;
  const hasChildren = childCount > 0;
  const displayText = n.ai_summary ?? (n.content.length > 100 ? n.content.slice(0, 100) + "\u2026" : n.content);
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
          ...
          {degree > 0 && (
            <span style={{ background: "rgba(99,102,241,0.15)", color: "#818cf8", fontSize: 9, padding: "1px 5px", borderRadius: 4, fontWeight: 600 }}>
              {degree} links
            </span>
          )}
        </div>
      ...
      <div style={{ padding: "8px 10px", minHeight: 50 }}>
        <p style={{ margin: 0, fontSize: 11, lineHeight: 1.55, color: "#e2e4e9" }}>{displayText}</p>
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 10, color: "#6b7080", display: "flex", alignItems: "center", gap: 4 }}>
          <span style={{ color: "#9ca0ab", fontWeight: 500 }}>{n.author_display_name}</span>
          {n.track_name && <span style={{ color: c.accent }}>{"\u00b7"} {n.track_name}</span>}
          <span style={{
            background: STATE_COLORS[n.state]?.accent ? `${STATE_COLORS[n.state].accent}20` : "#22262e",
    ...
    <div className="absolute right-0 top-0 bottom-0 w-80 bg-surface-1 border-l border-border z-50 overflow-y-auto animate-slide-in-right">
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={onClose} className="p-1 rounded hover:bg-surface-3 text-text-tertiary hover:text-text-secondary transition">
            <X size={14} />
          </button>
        </div>
        ...
        {node.ai_summary && (
          <div className="bg-accent-muted rounded-lg p-3 mb-3">
            <p className="text-[10px] font-semibold text-accent uppercase tracking-wider mb-1">AI Summary</p>
            <p className="text-xs text-text-secondary leading-relaxed">{node.ai_summary}</p>
          </div>
        )}
        ...
        <div className="space-y-2 text-xs mb-3">
          <div className="flex justify-between">
            ...
            <span className="text-text-primary font-medium">{node.author_display_name}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-text-tertiary">State</span>
            <span className="px-2 py-0.5 rounded text-[10px] font-medium"
              style={{ background: `${STATE_COLORS[node.state]?.accent}20`, color: STATE_COLORS[node.state]?.accent }}>
              {node.state}
            </span>
          </div>
          {node.track_name && (
          ...
          <div className="flex justify-between">
            ...
            <span className="text-text-secondary">{new Date(node.created_at).toLocaleDateString()}</span>
          </div>
        </div>
        ...
        {node.nuance_tags.length > 0 && (
          <div className="mb-3">
            <p className="text-[10px] font-semibold text-text-tertiary uppercase tracking-wider mb-1">Nuance Tags</p>
            <div className="flex flex-wrap gap-1">
              {node.nuance_tags.map((tag) => (
                <span key={tag} className="bg-surface-3 text-text-secondary text-[10px] px-2 py-0.5 rounded">
                  {tag.replace(/_/g, " ")}
                </span>
              ))}
            </div>
        ...
        <div className="mb-3">
          ...
          <div className="space-y-1">
            {connections.incoming.map((e) => {
              ...
              return (
                <div key={e.id} className="bg-surface-2 rounded px-2 py-1.5 text-[10px]">
                  <span className="font-medium" style={{ color: EDGE_COLORS[e.relationship_type] }}>
                    {e.relationship_type}
                  </span>
                  <span className="text-text-tertiary"> from </span>
                  <span className="text-text-secondary">{src?.author_display_name ?? "unknown"}</span>
                  {src && (
            ...
            {connections.outgoing.map((e) => {
              ...
              return (
                <div key={e.id} className="bg-surface-2 rounded px-2 py-1.5 text-[10px]">
                  ...
                  <span className="text-text-secondary">{tgt?.author_display_name ?? "unknown"}</span>
                  {tgt && (
                    <p className="text-text-tertiary mt-0.5 line-clamp-1">
...
function ExpandedMapInner({ graphNodes, graphEdges, tracks, onClose, onNodeClick }: Props) {
  ...
  const { visibleNodes, visibleEdges } = useMemo(() => {
    ...
    if (filterState !== "all") filtered = filtered.filter((n) => n.state === filterState);
    if (searchTerm.trim()) {
  ...
  }, [graphNodes, graphEdges, collapsed, filterTrack, filterType, filterState, searchTerm]);
  ...
  const rfNodes: Node[] = useMemo(() => {
    ...
    switch (layout) {
      case "hierarchy-tb": return applyDagreLayout(raw, rawEdges, "TB");
      case "hierarchy-lr": return applyDagreLayout(raw, rawEdges, "LR");
      case "radial": return applyRadialLayout(raw, rawEdges);
      case "cluster-track": return applyClusterLayout(raw, rawEdges, visibleNodes);
    }
  }, [visibleNodes, visibleEdges, collapsed, toggleCollapse, graphEdges, graphNodes, handleNodeClick, layout, colorMode, selectedNodeId, degreeMap]);
  ...
  const rfEdges: Edge[] = useMemo(() =>
    visibleEdges.map((e) => ({
      ...
      markerEnd: { type: MarkerType.ArrowClosed, color: EDGE_COLORS[e.relationship_type] ?? "#6b7080", width: 14, height: 14 },
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-surface-0 flex flex-col animate-fade-in" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
      {/* ── TOOLBAR ─────────────────────────────────────────────── */}
      <div className="h-11 bg-surface-1 border-b border-border flex items-center px-3 gap-2 shrink-0">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span className="text-xs font-semibold text-text-secondary uppercase tracking-wider shrink-0">Argument Map</span>
          <span className="text-text-tertiary text-xs">·</span>
          <span className="text-text-tertiary text-xs tabular-nums">{visibleNodes.length}/{graphNodes.length} nodes</span>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <select value={filterState} onChange={(e) => setFilterState(e.target.value)}
            ...
            {(Object.keys(STATE_COLORS) as ArgumentState[]).map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="relative">
            <button
              onClick={() => { setShowLayoutMenu(!showLayoutMenu); setShowColorMenu(false); }}
                ...
                {LAYOUT_OPTIONS.map((opt) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-1.5 text-xs transition ${
                      layout === opt.value ? "text-accent bg-accent-muted" : "text-text-secondary hover:bg-surface-3 hover:text-text-primary"
                    }`}
                  >
                    <opt.icon size={12} />
                    {opt.label}
          ...
          <div className="relative">
            <button
              onClick={() => { setShowColorMenu(!showColorMenu); setShowLayoutMenu(false); }}
              className="flex items-center gap-1 px-2 py-1 text-xs bg-surface-2 border border-border-subtle rounded-md text-text-secondary hover:text-text-primary hover:border-border-hover transition"
              title="Color mode"
            >
              <Palette size={12} />
            ...
            {showColorMenu && (
              <div className="absolute right-0 top-full mt-1 bg-surface-2 border border-border rounded-lg shadow-lg py-1 z-50 min-w-[180px] animate-scale-in">
                {COLOR_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    onClick={() => { setColorMode(opt.value); setShowColorMenu(false); }}
                    className={`w-full text-left px-3 py-1.5 text-xs transition ${
                      colorMode === opt.value ? "text-accent bg-accent-muted" : "text-text-secondary hover:bg-surface-3 hover:text-text-primary"
                    }`}
                  >
                    {opt.label}
                  </button>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className="flex-1 relative">
          <ReactFlow
            ...
            onClick={() => { setShowLayoutMenu(false); setShowColorMenu(false); }}
          >
            <Background color="#2a2f38" gap={24} size={1} />
            <Controls showInteractive={false} style={{ borderRadius: 8, boxShadow: "0 2px 12px rgba(0,0,0,0.4)" }} />
            <MiniMap
              nodeColor={(n) => {
                const gn = n.data?.graphNode as GraphNode | undefined;
                if (!gn) return "#6b7080";
                return getNodeColors(gn, colorMode, graphNodes, graphEdges).accent;
              }}
          ...
          <div className="absolute bottom-2 left-2 right-2 mx-auto max-w-3xl">
            <div className="bg-surface-1/90 backdrop-blur border border-border-subtle rounded-lg px-4 py-2 flex flex-wrap justify-center gap-x-4 gap-y-1 text-[10px]">
              {colorMode === "type" && (Object.entries(NODE_COLORS) as [NodeType, typeof NODE_COLORS[NodeType]][]).map(([type, c]) => (
              ...
              {colorMode === "state" && (Object.entries(STATE_COLORS) as [ArgumentState, typeof STATE_COLORS[ArgumentState]][]).map(([state, c]) => (
                <div key={state} className="flex items-center gap-1 text-text-tertiary">
                  <span className="w-2.5 h-2.5 rounded-sm" style={{ background: c.bg, border: `1px solid ${c.border}` }} />
                  <span>{state}</span>
                </div>
              ))}
              {colorMode === "age" && (
                <>
                  <span className="text-text-tertiary flex items-center gap-1">
                    <span className="w-8 h-2.5 rounded-sm" style={{ background: "linear-gradient(90deg, hsl(240,60%,30%), hsl(360,80%,55%))" }} />
                    Oldest → Newest
                  </span>
                </>
              )}
              {colorMode === "connectivity" && (
                <>
                  <div className="flex items-center gap-1 text-text-tertiary">
                    <span className="w-2.5 h-2.5 rounded-sm" style={{ background: "#3a3f4a" }} /> Low
                  </div>
                  <div className="flex items-center gap-1 text-text-tertiary">
                    <span className="w-2.5 h-2.5 rounded-sm" style={{ background: "#6366f1" }} /> Medium
                  </div>
                  <div className="flex items-center gap-1 text-text-tertiary">
                    <span className="w-2.5 h-2.5 rounded-sm" style={{ background: "#818cf8" }} /> High
                  </div>
                </>
              )}
              {/* Edge legend */}
              <div className="w-px h-3 bg-border-subtle self-center" />
              {(Object.entries(EDGE_COLORS) as [EdgeRelationship, string][]).map(([rel, color]) => (
                <div key={rel} className="flex items-center gap-1 text-text-tertiary">
            ...
            <div className="h-full flex flex-col">
              <div className="px-3 py-2.5 border-b border-border flex items-center justify-between shrink-0">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-semibold text-text-secondary uppercase tracking-wider">Analytics</span>
                </div>
                <button onClick={() => setShowAnalytics(false)} className="p-1 rounded hover:bg-surface-3 text-text-tertiary hover:text-text-secondary transition">
                  <X size={12} />
                </button>
              </div>
              <div className="flex-1 overflow-y-auto">
                <MapAnalytics
                  nodes={visibleNodes}
                  edges={visibleEdges}
                  onHighlightNode={(id) => { setSelectedNodeId(id); }}
                />
              </div>
            </div>
          </aside>
